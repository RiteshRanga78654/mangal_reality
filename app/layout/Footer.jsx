  ...
  return (
    <footer className="relative bg-[#0A1A10] text-[#E0E7E0] overflow-hidden pt-24 pb-12 border-t border-[#4ADE80]/20">
      ...
      >
        ...
        <div className="mt-24 pt-8 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8">
          <p className="text-[10px] uppercase tracking-[0.4em] text-gray-500 font-medium">
            © 2026 Mangal Realty — Sustainable Luxury.
          </p>
          
          <div className="flex items-center gap-6">
            {[FaTwitter, FaFacebookF, FaPinterestP, FaInstagram].map((Icon, i) => (
              <Link key={i} href="#" className="text-gray-500 hover:text-[#4ADE80] transition-all duration-300 transform hover:-translate-y-1">
                <Icon size={18} />
              </Link>
            ))}
          </div>
      ...
      <motion.button 
        ...
        whileTap={{ scale: 0.9 }}
        onClick={scrollToTop}
        className="fixed bottom-10 cursor-pointer right-10 w-14 h-14 bg-[#15803D] text-white rounded-full flex items-center justify-center shadow-[0_10px_30px_rgba(21,128,61,0.4)] z-40"
      >
        <ChevronUp size={24} />
      </motion.button>
    </footer>
