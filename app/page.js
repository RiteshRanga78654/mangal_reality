...
const Page = () => {
  ...
  // ✅ Auto-open ContactPopup after 10 seconds
  useEffect(() => {
  ...
  const PROJECTS = [
    ...
    },
    {
      ...
      status: "On Going",
      area: "30 Acres",
      image: "/assets/images/nature-valley-phase-1.jpeg",
  ...
  const nextProject = useCallback(() => {
    setProjectIndex((prev) =>
      prev + 1 >= PROJECTS.length - (projectsVisible - 1) ? 0 : prev + 1
    );
  }, [projectsVisible]);

  const prevProject = () => {
    setProjectIndex((prev) =>
      prev === 0 ? PROJECTS.length - projectsVisible : prev - 1
    );
  };
  ...
  return (
    <>
      {/* ✅ ContactPopup — auto-opens after 10 seconds */}
      <ContactPopup
          ...
          {STICKY_CONTENT.map((item, i) => (
            <div key={i} className="sticky top-0 h-screen w-full overflow-hidden">
              ...
              <div className="absolute inset-0 z-20 flex flex-col items-center justify-center text-center px-6 text-white">
                <span className="text-xl leading-relaxed mb-4 font-medium tracking-widest">{item.label}</span>
                <h2 className="text-4xl md:text-6xl font-bold tracking-tight mb-4">{item.title}</h2>
                <div className="w-20 h-1 bg-[#22C55E] mx-auto mb-6"></div>
                <p className="max-w-3xl text-lg md:text-xl font-medium leading-relaxed opacity-90">{item.desc}</p>
              </div>
            </div>
          ))}
        </section>
          ...
          <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
            <div className="space-y-6 text-center md:text-left">
              ...
              <p className="text-lg md:text-xl text-black/70 leading-relaxed max-w-xl">
                At Mangal Realty, we are built on <span>quality, trust, engineering excellence, ethical practices, and transparency</span>, making us one of the most preferred real estate brands.
              </p>
            ...
            <div className="grid grid-cols-2 gap-5">
              {stats.map((item, i) => (
                <div key={i} className="p-10 bg-white border border-stone-200 rounded-xl flex flex-col items-center text-center hover:shadow-2xl transition-all duration-500 group">
                  <div className="mb-3 text-4xl md:text-5xl font-bold group-hover:scale-110 transition-all duration-300" style={{ color: brandGreen }}>
                    <StatCounter endValue={item.number} />
                  </div>
                  <p className="font-semibold text-stone-800 text-xs tracking-widest leading-tight">{item.label}</p>
                </div>
              ))}
            </div>
          </div>
              ...
              <div className="overflow-hidden">
                ...
                >
                  {PROJECTS.map((project) => (
                    <div key={project.id} className="px-2 flex-shrink-0" style={{ width: `${100 / projectsVisible}%` }}>
                      <motion.div
                      ...
                      >
                        ...
                        <div className="absolute inset-0 flex flex-col justify-end p-8 md:p-10 pointer-events-none">
                          <motion.div animate={{ y: hoveredId === project.id ? 0 : 20, opacity: 1 }} className="space-y-4 pointer-events-auto">
                      ...
                      <div key={`${item.id}-${index}`} className="flex-shrink-0 px-4" style={{ width: `${100 / itemsPerPage}%` }}>
                        <div className="bg-[#f8fafc] p-6 h-[320px] rounded-none border border-slate-200 flex flex-col justify-between text-left relative group hover:border-[#22C55E]/50 transition-all">
                          ...
                          <div className="z-10 mt-4 overflow-y-auto pr-2 custom-scrollbar">
                            <p className="text-black text-sm md:text-base leading-relaxed">"{item.text}"</p>
                          </div>
                          <div className="flex items-center gap-4 pt-4 border-t border-slate-100 flex-shrink-0">
                            ...
                            <div className="flex-1">
                              <h4 className="text-[#0a1a10] font-bold text-sm md:text-base tracking-tight leading-none">{item.name}</h4>
                              <p className="text-[#22c55e] text-[10px] font-bold tracking-widest mt-1">{item.role}</p>
                            </div>
                          </div>
                        </div>
                      </div>
