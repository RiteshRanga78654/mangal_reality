...
const GreenMeadowsJonnada = () => {
  ...
  const MangalButton = ({ children, onClick, className = "" }) => (
    <button
      onClick={onClick}
      className={`group relative cursor-pointer px-10 py-5 text-white font-black tracking-[0.2em] text-[10px] md:text-xs overflow-hidden uppercase transition-all duration-300 w-full sm:w-auto ${className}`}
    >
  ...
  return (
    <div className="bg-[#f8fafc] min-h-screen font-sans antialiased text-slate-900 overflow-x-hidden">
      ...
      {/* 1. HERO SECTION */}
      <section className="relative min-h-screen flex flex-col lg:flex-row bg-[#0A1A10] overflow-hidden">
        ...
        <div className="w-full lg:w-1/3 flex items-center px-6 md:px-12 py-12 lg:py-20 bg-[#0A1A10]">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="w-full text-left">
            ...
            <h1 className="text-4xl md:text-6xl font-black text-white leading-none uppercase mb-6 tracking-tighter">
      ...
      {/* 2. STATS GRID */}
      <section className="px-6 -mt-10 md:-mt-16 relative z-30">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
          {projectStats.map((stat, i) => (
            <div key={i} className="bg-white shadow-xl p-6 md:p-10 border-b-4 md:border-b-8 border-[#22C55E] group text-left">
              ...
              <h2 className="text-2xl md:text-4xl font-black text-black tracking-tighter">{stat.value}</h2>
      ...
      <section className="py-16 md:py-24 px-6 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 md:mb-16 gap-6 text-left">
          <div className="w-full">
            <h2 className="text-4xl md:text-7xl font-black tracking-tighter leading-none uppercase">Project <br className="hidden md:block" /><span className="text-[#22C55E]">Highlights</span></h2>
            <div className="w-16 md:w-20 h-2 bg-[#22C55E] mt-4 md:mt-6"></div>
          </div>
          <p className="max-w-md text-slate-500 font-medium text-sm md:text-base">100% Work Completed. VMRDA Approved layout featuring premium amenities for modern living.</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          {highlights.map((item, idx) => (
            <div key={idx} className="bg-white p-6 md:p-8 border border-slate-100 shadow-sm hover:bg-[#0A1A10] hover:text-white transition-all duration-500 group text-left">
              <div className="text-[#22C55E] mb-6 group-hover:scale-110 transition-transform">{item.icon}</div>
              <h4 className="text-lg md:text-xl font-black uppercase mb-3 tracking-tight">{item.title}</h4>
      ...
      <section className="py-16 md:py-24 bg-[#0A1A10] px-6 text-white">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-12 md:mb-16 gap-8 text-left">
            <div className="max-w-2xl">
              <h2 className="text-4xl md:text-6xl font-black tracking-tighter leading-none uppercase">Location <span className="text-[#22C55E]">Corridor</span></h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
            <div className="md:col-span-2 h-[300px] md:h-[450px] relative overflow-hidden group">
              ...
              <div className="absolute bottom-6 left-6 text-left">
                <span className="bg-[#22C55E] text-black px-3 py-1 text-[9px] font-black uppercase mb-3 inline-block tracking-widest">Educational Hub</span>
                <h4 className="text-2xl md:text-4xl font-black uppercase">Lendi, Raghu & VITAM</h4>
              </div>
            </div>
            
            <div className="h-[300px] md:h-[450px] relative overflow-hidden group">
              <img src="https://images.unsplash.com/photo-1436491865332-7a61a109c0f2?auto=format&fit=crop&q=80&w=600" className="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-1000" alt="Airport" />
              <div className="absolute inset-0 bg-[#22C55E]/20 group-hover:bg-[#22C55E]/40 transition-colors"></div>
              <div className="absolute bottom-8 left-8 text-left">
                <h4 className="text-2xl md:text-3xl font-black uppercase text-white leading-none">15 KM TO <br /> AIRPORT</h4>
              </div>
            </div>
            ...
            <AnimatePresence>
              {showMoreLandmarks && (
                <>
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="h-[250px] md:h-[350px] relative overflow-hidden">
                    ...
                    <div className="absolute inset-0 bg-black/60 p-8 flex flex-col justify-end text-left">
                      <h4 className="text-xl md:text-2xl font-black uppercase text-[#22C55E]">Padmanabham</h4>
            ...
            <div className={`transition-all duration-700 bg-white p-6 md:p-12 flex flex-col justify-center text-left md:col-span-3 ${!showMoreLandmarks && 'lg:col-span-2'}`}>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6 w-full">
                {[{l: "Revidi Junction", d: "3 KM"}, {l: "Highway", d: "5 KM"}, {l: "Lake Palace", d: "9 KM"}, {l: "Madhurawada", d: "25 KM"}].map((item, i) => (
                  <div key={i} className="flex flex-col border-l-2 border-[#22C55E] pl-4">
                    ...
                    <span className="text-xl md:text-3xl font-black text-black">{item.d}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-20 items-center">
            <div className="relative text-left">
              ...
              <div className="relative z-10">
                <h3 className="text-4xl md:text-7xl font-black uppercase tracking-tighter leading-none mb-8">Investor <br /><span className="text-[#22C55E]">Voices</span></h3>
                ...
                <div className={`transition-all duration-500 min-h-[150px] ${isFading ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'}`}>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 md:w-16 md:h-16 bg-[#0A1A10] text-[#22C55E] rounded-full flex items-center justify-center font-black text-xl">
                      {testimonials[testimonialIndex].name.charAt(0)}
                    </div>
                    <div>
                      <p className="font-black text-lg md:text-xl uppercase tracking-tighter">{testimonials[testimonialIndex].name}</p>
      ...
      {/* 6. CONTACT FOOTER */}
      <footer className="py-16 md:py-24 px-6 bg-[#0A1A10] text-center text-white">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-3xl md:text-6xl lg:text-7xl font-black uppercase tracking-tighter mb-10 leading-none">Your Jonnada <br /> <span className="text-[#22C55E]">Legacy Starts</span> Here</h2>
            
            <div className="flex flex-col md:flex-row justify-center gap-6 md:gap-12 items-center mb-12">
               <div className="flex items-center gap-3 font-black uppercase tracking-[0.2em] text-[10px] md:text-xs">
